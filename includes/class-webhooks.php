...
class Webhooks {
    ...
    /**
     * Listens for incoming event data from Bullhorn
     *
     * @return void
     */
    public function webhooks_listener() {
        if ( $_SERVER['REQUEST_METHOD'] == 'POST' && isset( $_GET['job_manager_webhook'] ) && $_GET['job_manager_webhook'] == 'bullhorn' ) {
            // 
            exit;
        }
    }
    ...
    public function get_events() {
        return apply_filters( 'job_manager_bullhorn_webhooks_events', $this->events );
    }
    ...
    public function get_enabled_events() {
        $webhooks = WP_Job_Manager_Bullhorn()->clients['bullhorn']->adapter()->get_webhooks( array(), 'Enabled' );
        ...
        if ( ! is_wp_error( $webhooks ) ) {
            foreach ( $webhooks->items as $webhook ) {
                 ...
                 */
                if ( basename( parse_url( $webhook->url, PHP_URL_HOST ) ) !== basename( home_url() ) || strpos( parse_url( $webhook->url, PHP_URL_QUERY ), 'job_manager_webhook=bullhorn' ) === false ) {
                    continue;
                }
    ...
    public function setup() {
        return WP_Job_Manager_Bullhorn()->clients['bullhorn']->adapter()->post_webhook( 'wp_job_manager_bullhorn', $this->get_events(), 'Enabled' );
    }
    ...
    public function reset() {
        if ( sizeof( $this->webhook_ids ) > 1 ) {
            foreach ( $this->webhook_ids as $webhook_id ) {
                WP_Job_Manager_Bullhorn()->clients['bullhorn']->adapter()->delete_webhook( $webhook_id );
            }
